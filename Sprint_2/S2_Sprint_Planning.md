...
The current school map is a dedicated Leaflet-based school-map section inside the Gutenberg page shell, not a single full-page Custom HTML replacement and not a WP Go Maps shortcode block. Live-page inspection on 12 May showed the editable Sprint 2 shell, one `#s2-school-map` Leaflet container, search/radius/sector/type/area/suburb controls, nearest-six filtering, range-circle feedback, and route preview. This is a bounded risk response to the Sprint 1 component limits already recorded in the defect log: plugin configuration constraints, limited close-detail usability, and route/marker/category behaviour that should not be inferred from arbitrary free-text input. Without this adjustment, the larger Sprint 2 map could repeat those limits while also needing coordinate search, Melbourne Connect default with 1 km density control, nearest-results list, category filtering over the 913-record reduced dataset, sector marker colours, and enriched nearest-six popups. The rejected full-page prototype is retained only as historical comparison.
...
The reduction rule produced **913 open school records**, which is close to the expected "about 900 records" requirement. The repository evidence file keeps the fields needed for Sprint 2 map rendering, filtering, and QA: school name, sector, type, suburb, area, LGA, coordinates, distance from Melbourne Connect, and nearest-six enrichment fields. The current implementation path is an editable Gutenberg page shell plus a dedicated interactive school-map section, with component restore notes and import/filter CSV artefacts retained as baseline-protection evidence.
