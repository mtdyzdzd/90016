...
- The 12 May continuity check confirmed that Sprint 2 did not explicitly remove the Sprint 1 direction behaviour. The school-map widget therefore keeps a visible range circle and adds an in-page route preview from the current search centre to a selected school, while the Sprint 2 requirement change from university/library categories to school categories means the old campus/library markers are not part of the default school dataset view.
