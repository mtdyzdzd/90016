...
The **Test Date** column records the final close-out QA, retest, or evidence-reconciliation date. Earlier implementation and story-completion checkpoints are tracked in the Sprint Backlog and Burn-down Chart. This avoids treating the 2026-04-26 final QA pass as if all story work first happened on that date.

## Acceptance Criteria and Test Cases

| Test ID | User Story | Acceptance Criteria Checked | BDD Scenario | Result | Tested By | Test Date | Evidence / Comments |
| ------- | ---------- | --------------------------- | ------------ | ------ | --------- | --------- | ------------------- |
| S1-QA-001 | US-01 | Selected University of Melbourne and RMIT University building markers are displayed on the interactive map. | Given the WordPress map page is available, when the user opens the interactive map, then the selected building markers are displayed. | Pass | @Conghao Lin | 2026-04-26 | Old Arts Building and RMIT Building 80 are visible in `Sprint_1/etc/S1_Map_Overview.png`. Initial story checkpoint recorded on Apr 23; final evidence check completed on Apr 26. |
...
| Required library evidence follows the dataset reduction rule. | Met with documented evidence boundary | US-02 required evidence is limited to City Library as the City of Melbourne marker. The source check used the Data Vic / City of Melbourne business-establishments dataset, latest available year `2024`, and library classification `industry_anzsic4_code = 6010` / `Libraries and Archives`. Optional UoM/RMIT library markers are retained only as contextual map content. |
| Bugs, defects, and accepted constraints discovered during delivery were recorded and retested or explicitly accepted. | Met | `Main/Defect_Log_Tracker.md` records DEF-001 to DEF-006, including the marker-colour issue closed on 2026-04-26 and the accepted zoom/category constraints carried into Sprint 2. |
