...
## Follow-up Rule Added After Sprint 1 Feedback

The Sprint 1 feedback identified that previous action items should be followed up in later stand-ups. For this Sprint 1 close-out version, the Action Items Log below records final closure status for each action, and Sprint 2 stand-up records extend the format by adding an explicit "Previous Work / Follow-up" column at each checkpoint.
...
| @Conghao Lin | Reviewed which tasks were still missing QA, peer review, and validation evidence. | Treat US-03 and US-02 as story checkpoints requiring final evidence reconciliation, and keep US-04 out of Done discussion until the route workflow is stable. | Final QA evidence is not fully reconciled yet, and US-04 remains technically uncertain. | Track readiness for validation, keep checkpoint dates separate from final QA dates, and update backlog/status artefacts accordingly. |
