...
| S2-QA-014 | US-05 / US-06 / US-07 | Given generated evidence is used, when local files are reviewed, then data, import, filter, verification notes, and the CSV-generation helper script should be present.                       | Pass        | @Fazheng Xu | 2026-05-11 | Reduced CSV, nearest-six CSV, marker import CSV, filter model CSV, verification notes, and the CSV-generation helper script are stored under `Sprint_2/etc/`; raw backups and local previews are excluded from the formal evidence set. |
